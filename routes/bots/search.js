...
app.post("/bots/find/:bot", async (req, res) => {
    try {
        ...
        let { bot } = req.params;
        if (!bot) return error(res, 'Proszę podać ID bota lub nazwę bota.');
        if (bot.length < 3) return error(res, 'Nazwa lub ID bota musi wynosić co najmniej <strong>3</strong> znaki.');
        ...
        if (!foundBots) {
            try {
                ...
                return error(res, 'Przepraszamy, ale nie znaleźliśmy żadnego bota o takiej nazwie lub identyfikatorze.');
            }
        }

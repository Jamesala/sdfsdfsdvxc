    ...
    if (guild.members.cache.has(message.author.id)) {
      if (guild.members.cache.get(message.author.id).roles.cache.has(roles.botReviewer)) {
        ...
        if (botdata.status === "Approved") {
          return message.channel.send("Ten bot został już zatwierdzony.");
        }

        const guild1 = client.guilds.cache.get(config.server.id);
        if (!guild1.members.cache.has(botdata.botID)) {
          return message.channel.send(`Niestety, nie możesz zatwierdzić bota, który nie jest członkiem serwera głównego.\nProszę podać użytkownika, który ma uprawnienia administratora i może go zaprosić.`);
        }

        guild1.members.cache.get(botdata.botID).roles.add(roles.verifiedBot);

        client.users.fetch(bot.id).then((bota) => {
          client.channels.cache.get(config.server.channels.botlogs).send(`${global.config.server.emojis.approve ?? "✅"} | <@${bota.id}> od <@${botdata.ownerID}>${botdata.coowners?.length ? `, ${botdata.coowners.map(u => `<@${u}>`).join(', ')}` : ''} został zatwierdzony przez <@${message.author.id}>.\n<${global.config.website.url}/bot/${bota.id}>`);
          if (client.users.cache.get(botdata.ownerID)) {
            client.users.cache.get(botdata.ownerID).send(`${global.config.server.emojis.approve ?? "✅"} | Twój bot o nazwie **${bota.tag}** został zatwierdzony! :tada:`);
          ...
          for (coowner of botdata.coowners) {
            if (client.users.cache.get(coowner)) {
              client.users.cache.get(coowner).send(`${global.config.server.emojis.approve ?? "✅"} | Twój bot o nazwie **${bota.tag}** został zatwierdzony! :tada:`);
        ...
        message.channel.send("Bot został zatwierdzony szefie");
      }
    }
  },
};